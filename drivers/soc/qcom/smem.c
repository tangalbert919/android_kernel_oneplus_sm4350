...
//Add for dump reason
#ifdef CONFIG_OPLUS_FEATURE_AGINGTEST
...
void save_dump_reason_to_smem(char *reason, char *function_name)
{
	...
	if (IS_ERR_OR_NULL(dp_info))
		...
		if ((reason != NULL) && ((strlen(dp_info->dump_reason) + reason_len) < DUMP_REASON_SIZE))
		...
		if ((function_name != NULL) && ((strlen(dp_info->dump_reason) + name_len + 1) < DUMP_REASON_SIZE))
		{
	...
	pr_err("\r%s: dump_reason : %s reason_len=%d function caused panic :%s name_len=%d \n", __func__,
							reason, reason_len, function_name, name_len);
