...
static int pmicwd_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if((pon->suspend_state & 0x0F) >= 1)
			panic("oplus suspend resume state %d\n", pon->suspend_state);
		...
		size_t count,loff_t *off)
{
	...
	if(tmp_timeout < 60 || tmp_timeout > 255){
		tmp_timeout = OPLUS_PMIC_WD_DEFAULT_TIMEOUT;
	}
...
void oplus_set_pmicWd_state(int enable)
{
	...
	dev_info(pon->dev, "oplus_set_pmicWd_state:old:%x new:%x\n", pon->pmicwd_state,new_state);
	if(new_state == pon->pmicwd_state)
		return ;
...
void pmicwd_init(struct platform_device *pdev, struct qpnp_pon *pon, bool sys_reset)
{
	...
	if(sys_reset && pon->pmicwd_state){
		if (get_eng_version() == AGING) {
			pon->pmicwd_state = OPLUS_PMIC_WD_DEFAULT_ENABLE | (OPLUS_PMIC_WD_DEFAULT_TIMEOUT << 8) |
		...
		#if OPLUS_PMIC_WD_DEFAULT_ENABLE
		pon->wd_task = kthread_create(pmicwd_kthread, pon,"pmicwd");
		if(pon->wd_task){
			if (get_eng_version() == AGING) {
				qpnp_pon_wd_timer(OPLUS_PMIC_WD_DEFAULT_TIMEOUT, PON_POWER_OFF_WARM_RESET);
			} else {
				qpnp_pon_wd_timer(OPLUS_PMIC_WD_DEFAULT_TIMEOUT, PON_POWER_OFF_HARD_RESET);
			}
			oplus_qpnp_pon_wd_config(1);
			wake_up_process(pon->wd_task);
		}else{
