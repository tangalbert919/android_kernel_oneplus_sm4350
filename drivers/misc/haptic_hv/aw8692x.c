				   ...
				   uint8_t val)
{
	aw_info("enter");
	if (addr == 1) {
		haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_PWMCFG2,
		...
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_PWMCFG4, &val,
				     AW_I2C_BYTE_ONE);
	}
}
...
static void aw8692x_cont_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_CONTCFG6,
				 ...
				 AW8692X_BIT_CONTCFG6_TRACK_DISABLE);
	/* f0 driver level */
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_CONTCFG6,
...
static uint8_t aw8692x_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_PWMCFG2, &reg_val,
			    AW_I2C_BYTE_ONE);
	reg_val &= 0x08;
	return reg_val;
}
...
static void aw8692x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_SYSCTRL5, reg_val,
			     AW_I2C_BYTE_ONE);
	/* Cont drv2 lvl */
	aw_haptic->info.cont_drv2_lvl = AW8692X_DRV2_LVL_FARMULA(
						 aw_haptic->info.cont_lra_vrms);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_PWMCFG1, reg_val,
			     AW_I2C_BYTE_ONE);
	aw8692x_reg_unlock(aw_haptic, false);
